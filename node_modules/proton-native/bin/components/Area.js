...
var onKey = function onKey(component) {
  return function (area, event) {
    ...
    if (extKey) {
      ...
      try {
        for (var _iterator = Object.keys(_libuiNode2.default.extKeys)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var k = _step.value;
    ...
    if (modifierKey) {
      ...
      try {
        for (var _iterator2 = Object.keys(_libuiNode2.default.modifierKeys)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          var _k = _step2.value;
    ...
    if (modifiers) {
      ...
      try {
        for (var _iterator3 = Object.keys(_libuiNode2.default.modifierKeys)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
...
var Area = function (_DesktopComponent) {
  ...
  function Area(root, props) {
    ...
    var _this = _possibleConstructorReturn(this, (Area.__proto__ || Object.getPrototypeOf(Area)).call(this, root, props));

    _this.root = root;
    _this.props = _extends({}, props);
    _this.setDefaults(props);

    _this.element = new _libuiNode2.default.UiArea(function (area, p) {
      for (var i = 0; i < _this.children.length; i += 1) {
        if (_typeof(_this.children[i]) === 'object') {
          _this.children[i].render(_this, area, p);
        }
      }
    }, onMouse(_this), function (area, inOut) {
      if (inOut === 0) {
        _this.props.onMouseEnter();
      } else {
        _this.props.onMouseLeave();
      }
    }, function dragBroken() {}, onKey(_this));
    return _this;
  }
...
Area.defaultProps = _extends({}, _DesktopComponent2.universalDefaultProps, {
  ...
  onMouseLeave: function onMouseLeave(area) {},
  onKeyUp: function onKeyUp(area, event) {},
  onKeyDown: function onKeyDown(area, event) {}
});
...
function toLibuiColor(color) {
  return new _libuiNode2.default.Color(color.red() / 255, color.green() / 255, color.blue() / 255, color.alpha());
}
...
var AreaComponent = function () {
  ...
  _createClass(AreaComponent, [{
    ...
    value: function applyTransforms(p) {
      ...
      try {
        for (var _iterator5 = this.props.transform.match(/\w+\([^)]+\)/g)[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
          var transform = _step5.value;

          // rotate(deg [,x, y])
          // default x: 50%, y: 50%
          var rotate = transform.match(/rotate\s*\(\s*([-0-9.]+)(?:\s*,\s*([-0-9.%]+)\s*,\s*([-0-9.%]+))?\s*\)/);
          if (rotate) {
            var xy = this.selfToParent(fallback(rotate[2], '50%', function (v) {
              return v;
          ...
          // translate(x [y])
          // default y: x
          var translate = transform.match(/translate\s*\(\s*([-0-9.%]+)(?:\s*,\s*([-0-9.%]+))?\s*\)/);
          if (translate) {
            mat.translate(this.parseSelf(translate[1], p), fallback(translate[2], translate[1], function (v) {
          ...
          var scale = transform.match(/scale\s*\(([-0-9.]+)(?:(?:\s*,\s*([-0-9.]+))?(?:\s*,\s*([-0-9.%]+)\s*,\s*([-0-9.%]+))?)?\)/);
          if (scale) {
            var _xy = this.selfToParent(fallback(scale[3], '50%', function (v) {
              return v;
            }), fallback(scale[4], '50%', function (v) {
              return v;
            }), p);
            if (process.platform === 'win32') {
              mat.scale(_xy.x, _xy.y, Number(scale[1]), fallback(scale[2], scale[1]));
            } else {
          ...
          // a, b: x/y angle
          // default x=y: 50%
          var skew = transform.match(/skew\s*\(\s*([-0-9.]+)\s*,\s*([-0-9.]+)(?:,\s*([-0-9.%]+),\s*([-0-9.%]+))?\)/);
          if (skew) {
            var rad1 = Number(skew[1]) * (Math.PI / 180);
            var rad2 = Number(skew[2]) * (Math.PI / 180);
            mat.skew(fallback(skew[2], '50%', function (v) {
          ...
          var matrix = transform.match(/matrix\s*\(\s*([-0-9.]+)\s*,\s*([-0-9.]+)\s*,\s*([-0-9.]+)\s*,\s*([-0-9.]+)\s*,\s*([-0-9.]+)\s*,\s*([-0-9.]+)\s*\)/);
          if (matrix) {
            mat.setM11(matrix[1]);
            mat.setM12(matrix[2]);
            mat.setM21(matrix[3]);
    ...
    key: 'render',
    value: function render(parent, area, p, props) {
      this.parent = parent;
      ...
      var path = this.draw(area, p, props);

      if (path) {
        var fillBrush = props.fill && props.fill != 'none' && createBrush((0, _color2.default)(props.fill), Number(props.fillOpacity));
        var strokeBrush = props.stroke && props.stroke != 'none' && createBrush((0, _color2.default)(props.stroke), Number(props.strokeOpacity));
...
Area.Group = function (_AreaComponent) {
  ...
  function AreaGroup(root, props) {
    ...
    var _this3 = _possibleConstructorReturn(this, (AreaGroup.__proto__ || Object.getPrototypeOf(AreaGroup)).call(this, root, props));
  ...
  _createClass(AreaGroup, [{
    ...
    key: 'draw',
    value: function draw(area, p, props) {
...
Area.Rectangle = function (_AreaComponent2) {
  ...
  _createClass(Rectangle, [{
    ...
    key: 'draw',
    value: function draw(area, p) {
      var path = new _libuiNode2.default.UiDrawPath(_libuiNode2.default.fillMode.winding);
      path.addRectangle(this.parseParent(this.props.x, p), this.parseParent(this.props.y, p, true), this.parseParent(this.props.width, p), this.parseParent(this.props.height, p, true));
      path.end();
      return path;
    }
  }]);
...
Area.Rectangle.propTypes = _extends({}, AreaComponentPropTypes, {
  ...
  y: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
...
Area.Line = function (_AreaComponent3) {
  ...
  _createClass(Line, [{
    key: 'getWidth',
    value: function getWidth(p) {
      return Math.abs(this.parseParent(this.props.x2, p) - this.parseParent(this.props.x1, p));
    }
  }, {
    key: 'getHeight',
    value: function getHeight(p) {
      return Math.abs(this.parseParent(this.props.y2, p, true) - this.parseParent(this.props.y1, p, true));
    }
  }, {
    key: 'draw',
    value: function draw(area, p) {
      var path = new _libuiNode2.default.UiDrawPath(_libuiNode2.default.fillMode.winding);
      path.newFigure(this.parseParent(this.props.x1, p), this.parseParent(this.props.y1, p, true));
      path.lineTo(this.parseParent(this.props.x2, p), this.parseParent(this.props.y2, p, true));
      path.end();
...
Area.Line.propTypes = _extends({}, AreaComponentPropTypes, {
  x1: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  y1: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  x2: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  y2: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired
});
...
Area.Arc = function (_AreaComponent4) {
  ...
  _createClass(Arc, [{
    ...
    value: function draw(area, p) {
      var path = new _libuiNode2.default.UiDrawPath(_libuiNode2.default.fillMode.winding);
      path.newFigureWithArc(this.parseParent(this.props.x, p), this.parseParent(this.props.y, p, true), this.parseParent(this.props.r, p), Number(this.props.start) * (Math.PI / 180), Number(this.props.sweep) * (Math.PI / 180), false);
      path.end();
      return path;
    }
  }]);
...
Area.Arc.propTypes = _extends({}, AreaComponentPropTypes, {
  x: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  y: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  r: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  start: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]),
  sweep: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired
});
...
Area.Circle = function (_AreaComponent5) {
  ...
  _createClass(Circle, [{
    ...
    value: function draw(area, p) {
      var path = new _libuiNode2.default.UiDrawPath(_libuiNode2.default.fillMode.winding);
      path.newFigureWithArc(this.parseParent(this.props.x, p), this.parseParent(this.props.y, p, true), this.parseParent(this.props.r, p), 0, 2 * Math.PI, false);
      path.end();
      return path;
    }
  }]);
...
Area.Bezier = function (_AreaComponent6) {
  ...
  _createClass(Bezier, [{
    ...
    value: function draw(area, p) {
      var path = new _libuiNode2.default.UiDrawPath(_libuiNode2.default.fillMode.winding);
      path.newFigure(this.parseParent(this.props.x1, p), this.parseParent(this.props.y1, p, true));
      path.bezierTo(this.parseParent(this.props.cx1, p), this.parseParent(this.props.cy1, p, true), this.parseParent(this.props.cx2, p), this.parseParent(this.props.cy2, p, true), this.parseParent(this.props.x2, p), this.parseParent(this.props.y2, p, true));
      path.end();
...
Area.Bezier.propTypes = _extends({}, AreaComponentPropTypes, {
  x1: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  y1: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  cx1: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  cy1: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  x2: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  y2: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  cx2: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired,
  cy2: _propTypes2.default.oneOfType([_propTypes2.default.number, _propTypes2.default.string]).isRequired
});
...
Area.Path = function (_AreaComponent7) {
  ...
  _createClass(Path, [{
    key: 'draw',
    value: function draw(area, p) {
      var path = new _libuiNode2.default.UiDrawPath(this.props.fillRule === 'evenodd' ? _libuiNode2.default.fillMode.alternate : _libuiNode2.default.fillMode.winding);
      ...
      for (var i = 0; i < commands.length; i++) {
        ...
        switch (c.command) {
          ...
          default:
            ...
            throw new Error('Not implemented in Area.Path - ' + c.code + ': ' + c.command);
        }
      }
...
Area.Text = function (_AreaComponent8) {
  ...
  function AreaText(root, props) {
    ...
    var _this10 = _possibleConstructorReturn(this, (AreaText.__proto__ || Object.getPrototypeOf(AreaText)).call(this, root, props));
  ...
  _createClass(AreaText, [{
    ...
    value: function appendText(t) {
      for (var _len2 = arguments.length, attr = Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
    ...
    value: function render(parent, area, p, props) {
      ...
      var parentStyle = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};
      ...
      var attrs = Object.keys(style).map(function (k) {
        var v = style[k];
        switch (k) {
          case 'color':
            var color = (0, _color2.default)(v);
            return _libuiNode2.default.FontAttribute.newColor(toLibuiColor((0, _color2.default)(v)));
          case 'fontSize':
            return _libuiNode2.default.FontAttribute.newSize(Number(v));
          case 'fontFamily':
            return _libuiNode2.default.FontAttribute.newFamily(v);
          case 'backgroundColor':
            return _libuiNode2.default.FontAttribute.newBackgroundColor(toLibuiColor((0, _color2.default)(v)));
          case 'fontStyle':
            if (v in _libuiNode2.default.textItalic) {
              return _libuiNode2.default.FontAttribute.newItalic(_libuiNode2.default.textItalic[v]);
            }
            break;
          case 'fontWeight':
            if (typeof v === 'string' && v in _libuiNode2.default.textWeight) {
              return _libuiNode2.default.FontAttribute.newWeight(_libuiNode2.default.textWeight[v]);
            } else if (Number(v) >= _libuiNode2.default.textWeight.minimum && Number(v) <= _libuiNode2.default.textWeight.maximum) {
              return _libuiNode2.default.FontAttribute.newWeight(Number(v));
            }
            break;
          case 'textStretch':
            if (v in _libuiNode2.default.textStretch) {
              return _libuiNode2.default.FontAttribute.newStretch(_libuiNode2.default.textStretch[v]);
            }
            break;
          case 'textUnderline':
            if (v in _libuiNode2.default.textUnderline) {
              return _libuiNode2.default.FontAttribute.newUnderline(_libuiNode2.default.textUnderline[v]);
            }
            break;
          case 'textUnderlineColor':
            if (v !== 'custom' && v in _libuiNode2.default.textUnderlineColor) {
              return _libuiNode2.default.FontAttribute.newUnderlineColor(_libuiNode2.default.textUnderlineColor[v]);
            } else {
              return _libuiNode2.default.FontAttribute.newUnderlineColor(_libuiNode2.default.textUnderlineColor[v], toLibuiColor((0, _color2.default)(v)));
            }
        }
      }).filter(function (x) {
        return x;
      ...
      this.children.forEach(function (v) {
        if (typeof v === 'string') {
          _this11.appendText.apply(_this11, [v].concat(_toConsumableArray(attrs)));
        } else {
          v.render(_this11, area, p, props, style);
        }
      });
      ...
      if (!(this.parent instanceof AreaText)) {
        ...
        var font = new _libuiNode2.default.FontDescriptor('Arial', 12, _libuiNode2.default.textWeight.normal, _libuiNode2.default.textItalic.normal, _libuiNode2.default.textStretch.normal);

        var layout = new _libuiNode2.default.DrawTextLayout(this.str, font, p.getAreaWidth() - this.parseParent(this.props.x, p, false), textAlign);

        if (this.props.transform) {
          this.applyTransforms(p);
        }

        p.getContext().text(this.parseParent(this.props.x, p, false), this.parseParent(this.props.y, p, true), layout);
...
function areaProp(props, propName, componentName) {
  ...
  if (!(typeof v === 'string' || v.type === 'AREATEXT' || v.type === _.StyledText)) {
    return new Error('Invalid prop `' + propName + '` supplied to' + ' `' + componentName + '`. Has to be a string or an Area.Text component.');
  }
}

Area.Text.propTypes = {
  children: _propTypes2.default.oneOfType([areaProp, _propTypes2.default.arrayOf(areaProp)]),
